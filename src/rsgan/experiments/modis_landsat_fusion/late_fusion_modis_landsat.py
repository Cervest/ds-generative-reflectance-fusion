...
class LateFusionMODISLandsat(ImageTranslationExperiment):
    ...
    def on_epoch_end(self):
        ...
        if self.current_epoch == 0:
            ...
            self.logger.log_images(process_tensor_for_vis(landsat[:, [0, 2, 3]], 33, 98), tag='Source - Landsat (B4-B2-B3)', step=self.current_epoch)
            self.logger.log_images(process_tensor_for_vis(modis[:, [0, 2, 3]], 0, 98), tag='Source - MODIS (B1-B3-B4)', step=self.current_epoch)
    ...
    def test_step(self, batch, batch_idx):
        ...
        """
        # Unfold batch
        (landsat, modis), target, annotation = batch
